...
<body class="p-2">
    ...
    <table class="table table-bordered table-hover text-center border-dark">
        ...
        <tr>
            ...
            @endif
            <td>{{$tp->num_depot}}</td>
            <td style="vertical-align: middle"><a href="{{route('tp.destroy' , $tp)}}" class="btn btn-danger" onclick="return confirm('Vous-etre sure de supprimer cette donnée?')">supprimer</a></td>
            <td style="vertical-align: middle"><a href="{{route('tp.update' , $tp)}}" class="btn btn-primary">modifier</a></td>
        </tr>
        @empty
            @php
                $empty=true;
